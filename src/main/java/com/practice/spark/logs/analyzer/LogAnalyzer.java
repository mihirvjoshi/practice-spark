package com.practice.spark.logs.analyzer;
...
public class LogAnalyzer {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("Before execution => "+LocalTime.now());
    JavaRDD<ApacheAccessLog> accessLogs =
       logLines.map(ApacheAccessLog::parseFromLogLine).cache();
    System.out.println("After accesslogs execution => "+LocalTime.now());
    ...
    System.out.println("After content size execution => "+LocalTime.now());
    
    System.out.println(String.format("Content Size Avg: %s, Min: %s, Max: %s",
       contentSizes.reduce(SUM_REDUCER) / contentSizes.count(),
       contentSizes.min(Comparator.naturalOrder()),
       contentSizes.max(Comparator.naturalOrder())) + "at time " + LocalTime.now());
    ...
    List<Tuple2<Integer, Long>> responseCodeToCount =
        accessLogs.mapToPair(log -> new Tuple2<>(log.getResponseCode(), 1L))
            ...
            .take(100);
    System.out.println(String.format("Response code counts: %s", responseCodeToCount) + " at time " + LocalTime.now());
    ...
    List<String> ipAddresses =
        accessLogs.mapToPair(log -> new Tuple2<>(log.getIpAddress(), 1L))
            ...
            .map(Tuple2::_1)
            .take(100);
    System.out.println(String.format("IPAddresses > 10 times: %s", ipAddresses)+ " at time " + LocalTime.now());
    ...
    System.out.println(String.format("Top Endpoints: %s", topEndpoints)+ " at time " + LocalTime.now());
